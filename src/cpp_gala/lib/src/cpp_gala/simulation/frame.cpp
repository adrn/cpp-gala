...
void ConstantRotatingFrame::get_dwdt(double *w, double t, double *dwdt) {
    ...
    // Omega x q
    Cx = this->Omega[1] * w[2] - this->Omega[2] * w[1];
    Cy = -this->Omega[0] * w[2] + this->Omega[2] * w[0];
    Cz = this->Omega[0] * w[1] - this->Omega[1] * w[0];
    dwdt[0] += -Cx;
    dwdt[1] += -Cy;
    dwdt[2] += -Cz;

    // Omega x p
    Cx = this->Omega[1] * w[2+n_dim] - this->Omega[2] * w[1+n_dim];
    Cy = -this->Omega[0] * w[2+n_dim] + this->Omega[2] * w[0+n_dim];
    Cz = this->Omega[0] * w[1+n_dim] - this->Omega[1] * w[0+n_dim];
    dwdt[3] += -Cx;
    dwdt[4] += -Cy;
    dwdt[5] += -Cz;
}
